...
class ConventionChecker:
        ...
        if (
            ...
            codes = {
                ...
                Method: (
                    ...
                    else (violations.D107() if definition.is_init else violations.D102())
                ),
        ...
        if docstring:
            ...
            if not all(blanks_after) and blanks_after_count != 0:
                ...
                if not (
                    ...
                    and re(r"\s+(?:(?:class|def)\s|@)").match(after)
                ):
        ...
        if docstring and not function.is_test:
            ...
            if stripped:
                ...
                if correct_forms and check_word not in correct_forms:
                    best = max(
                        correct_forms, key=lambda f: common_prefix_length(check_word, f)
                    )
    ...
    def _is_docstring_section(context):
        ...
        return (
            this_line_looks_like_a_section_name and prev_line_looks_like_end_of_paragraph
        )
        ...
        if not dash_line_found:
            ...
            if non_empty_line.strip() != "-" * len(section_name):
                yield violations.D409(
                    len(section_name), section_name, len(non_empty_line.strip())
                )
