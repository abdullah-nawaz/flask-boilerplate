...
def do_update(
    ...
    while last is None or not last.use(now):
        ...
        dest = acquire.download_wheel(
            ...
            version_spec=None
            if last is None
            else "<{}".format(Wheel(Path(last.filename)).version),
...
def _get_release_date(dest):
    ...
    try:
        with urlopen(
            "https://pypi.org/pypi/{}/json".format(wheel.distribution)
        ) as file_handler:
            content = json.load(file_handler)
        return datetime.strptime(
            content["releases"][wheel.version][0]["upload_time"], "%Y-%m-%dT%H:%M:%S"
        )
...
def manual_upgrade(app_data):
    ...
    for for_py_version, distribution_to_package in BUNDLE_SUPPORT.items():
        ...
        for distribution in distribution_to_package.keys():
            thread = Thread(
                target=_run_manual_upgrade, args=(app_data, distribution, for_py_version)
            )
            thread.start()
