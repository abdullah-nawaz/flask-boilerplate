...
class Session(object):
    ...
    def _build_venvs(self):
        try:
            need_to_run = OrderedDict(
                (v, self.getvenv(v)) for v in self._evaluated_env_list
            )
            try:
                venv_order = stable_topological_sort(
                    OrderedDict(
                        (name, v.envconfig.depends) for name, v in need_to_run.items()
                    ),
                )
    ...
    def getvenv(self, name):
        ...
        if env_config is None:
            ...
            reporter.error(
                "venv {!r} in {} would delete project".format(name, env_config.envdir)
            )
    ...
    def _evaluated_env_list(self):
        ...
        tox_env_filter_re = (
            re.compile(tox_env_filter) if tox_env_filter is not None else None
        )
        visited = set()
    ...
    def _add_parallel_summaries(self):
        if (
            self.config.option.parallel != PARALLEL_OFF
            and "testenvs" in self.resultlog.dict
        ):
