        ...
        class HelpFormatter(argparse.ArgumentDefaultsHelpFormatter):
            def __init__(self, prog):
                super(HelpFormatter, self).__init__(
                    prog, max_help_position=35, width=190
                )
                ...
                for arg in config.option.args:
                    if arg and not os.path.isabs(arg):
                        ...
                        if os.path.exists(origpath):
                            arg = os.path.relpath(
                                origpath, testenv_config.changedir.strpath
                            )
                    args.append(arg)
...
def get_version_info(pm):
    ...
    if plugin_dist_info:
        ...
        for mod, egg_info in plugin_dist_info:
            ...
            out.append(
                "    {}-{} at {}".format(egg_info.project_name, egg_info.version, source)
            )
    return "\n".join(out)
...
def _split_factor_expr(expr):
    def split_single(e):
        raw = e.split("-")
        included = {
            _base_factor_name(factor) for factor in raw if not _is_negated_factor(factor)
        }
        excluded = {
            _base_factor_name(factor) for factor in raw if _is_negated_factor(factor)
        }
...
class SectionReader:
    def __init__(
        self, section_name, cfgparser, fallbacksections=None, factors=(), prefix=None
    ):
        if prefix is None:
    ...
    def getfloat(self, name, default=None, replace=True):
        ...
        if s is None:
            raise KeyError(
                "no config value [{}] {} found".format(self.section_name, name)
            )
    ...
    def getbool(self, name, default=None, replace=True):
        ...
        if not isinstance(s, bool):
            if s.lower() == "true":
                ...
                raise tox.exception.ConfigError(
                    "{}: boolean value {!r} needs to be 'True' or 'False'".format(
                        name, s
                    ),
                )
        return s
    ...
    def getstring(
        self, name, default=None, replace=True, crossonly=False, no_fallback=False
    ):
        x = None
    ...
    def words(self):
        ...
        def word_has_ended():
            return (
                ...
                or (
                    cur_char not in string.whitespace
                    and ps.word
                    and ps.word.strip() == ""
                )
            )
