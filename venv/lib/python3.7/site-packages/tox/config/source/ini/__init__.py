    ...
    def _discover_tox_envs(self, core_config):
        ...
        for section in self._parser.sections():
            if section.startswith(BASE_TEST_ENV):
                ...
                name = (
                    BASE_TEST_ENV if is_base_section else section[len(TEST_ENV_PREFIX) :]
                )
                if not is_base_section:
                    yield name
                if known_factors is None:
                    known_factors = set(
                        chain.from_iterable(e.split("-") for e in explicit)
                    )
...
class IniLoader(Loader, StrConvert):
    ...
    def _load_raw_from(self, as_name, conf, key):
        ...
        factor_selected = filter_for_env(
            replace_executed, as_name
