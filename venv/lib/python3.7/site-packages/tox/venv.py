...
class CreationConfig:
    ...
    def writeconfig(self, path):
        lines = [
            "{} {}".format(
                self.base_resolved_python_sha256, self.base_resolved_python_path
            ),
            "{} {:d} {:d} {:d}".format(
    ...
    def readconfig(cls, path):
        try:
            ...
            tox_version, sitepackages, usedevelop, alwayscopy = lines.pop(0).split(
                None, 4
            )
...
class VirtualEnv(object):
    ...
    def is_allowed_external(self, p):
        ...
        if tox.INFO.IS_WIN:
            tryadd += [
                os.path.normcase(x) for x in os.environ["PATHEXT"].split(os.pathsep)
            ]
    ...
    def update(self, action):
        ...
        if self.envconfig.recreate:
            ...
            if rconfig is None:
                ...
                outcome, reason = rconfig.matches_with_reason(
                    live_config, deps_subset_match
                )
        if reason is None:
        ...
        try:
            ...
            return "could not install deps {}; v = {!r}".format(
                self.envconfig.deps, exception
            )
    ...
    def test(
        ...
        with self.new_action(name) as action:
            ...
            if display_hash_seed:
                ...
                action.setactivity(
                    name, "PYTHONHASHSEED={!r}".format(env.get("PYTHONHASHSEED"))
                )
        ...
        with self.new_action("getenv", self.envconfig.envdir) as action:
            ...
            if status:
                ...
                command_log.add_command(
                    ["setup virtualenv"], str_status, default_ret_code
                )
                self.status = status
...
def tox_runenvreport(venv, action):
    ...
    output = venv._pcall(
        args, cwd=venv.envconfig.config.toxinidir, action=action, returnout=True
    )
