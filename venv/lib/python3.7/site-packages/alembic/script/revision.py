...
class RevisionMap(object):
    ...
    def _add_branches(self, revision, map_, map_branch_labels=True):
        ...
        if revision.branch_labels:
            ...
            parent = node
            while (
                parent and not parent._is_real_branch_point and not parent.is_merge_point
            ):
    ...
    def add_revision(self, revision, _replace=False):
        ...
        for downrev in revision._all_down_revisions:
            if downrev not in map_:
                util.warn(
                    "Revision %s referenced from %s is not present" % (downrev, revision)
                )
        ...
        if revision.is_head:
            self.heads = tuple(
                ...
                if head
                not in set(revision._versioned_down_revisions).union([revision.revision])
            ) + (revision.revision,)
